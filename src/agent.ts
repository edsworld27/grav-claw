...
// Session initialization: Load SOUL.md, USER.md, and CONSTITUTION.md for priority context
let soulContext = '';
let userContext = '';
...
function loadSessionContext(): void {
    ...
    // Load CONSTITUTION.md (Immutable Laws)
    const constitutionPath = path.join(dataDir, 'CONSTITUTION.md');
    if (fs.existsSync(constitutionPath)) {
        constitutionContext = fs.readFileSync(constitutionPath, 'utf8');
        console.log('[Session] Loaded CONSTITUTION.md');
    }
}
...
const getSystemPrompt = (senderId: string = 'default', conversationHistory?: ConversationMessage[]) => {
    ...
    // Feature: Omega Learning Ledger
    const LEARNINGS_FILE = path.join(__dirname, '../data', 'source_of_truth', 'LEARNINGS.md');
    let learningContext = '';
    if (fs.existsSync(LEARNINGS_FILE)) {
        learningContext = `\n\n### Omega Learning Ledger\nThe following are highly distilled heuristics, rules, and optimized patterns learned from past executions. You MUST integrate these into your behavior:\n\n${fs.readFileSync(LEARNINGS_FILE, 'utf-8')}`;
    }

    // Build the full system context with rigorous constitutional hierarchy
    const fullSystemPrompt = `${constitutionContext ? `\n${constitutionContext}\n\n` : ''}${soulContext ? `### Agent Identity\n${soulContext}\n\n` : ''}${userContext ? `### Operator Profile\n${userContext}\n\n` : ''}${CORE_SYSTEM_INSTRUCTIONS}
    ...
    while (iterations < MAX_ITERATIONS) {
        ...
        if (response.stopReason === 'tool_use' && response.toolCalls) {
            ...
            for (const toolCall of response.toolCalls) {
                ...
                try {
                    let result = await toolRegistry.executeTool(toolCall.name, toolCall.input, senderId);

                    // Feature: Strict Tool Output Truncator (The "Snipper")
                    if (typeof result === 'string' && result.length > 2500) {
                        console.warn(`[Agent] Truncating massive tool output from ${toolCall.name} (${result.length} chars)`);
                        result = result.substring(0, 2500) + `\n\n...[TRUNCATED BY STRICT TOOL SNIPPER: Output exceeded 2500 characters. To read more, refine your query or use a more specific search.]`;
                    }
            ...
            // SECURITY: Canary Tripwire Check
            if (CanaryToken.isCompromised(finalResponse)) {
                console.error(`[CRITICAL SECURITY] CANARY TRIPWIRE TRIGGERED! The agent outputted the forbidden token. Intervention required.`);
                SupabaseLogger.logEvent('security_warning', platform, senderId, 'CANARY TRIGGERED - Agent Compromised').catch(() => { });
                return "🚨 [SECURITY INTERVENTION]: The core agent's cognitive integrity was compromised by a hostile payload. Output has been wiped and the agent has been suspended.";
            }
...
export async function processUserMessage(
    ...
    // SECURITY: Pre-Flight LLM Firewall Check
    console.log(`[Security] Triggering Pre-Flight LLM Firewall...`);
    const firewallCheck = await LLMFirewall.preFlightCheck(userText);
    if (!firewallCheck.isSafe) {
        console.warn(`[Security] LLM Firewall blocked user input: ${firewallCheck.reason}`);
        SupabaseLogger.logEvent('security_warning', platform, senderId, 'Pre-Flight LLM Firewall blocked prompt injection.').catch(() => { });
        return "🚨 [SECURITY FIREWALL]: Your request was classified as a hostile prompt injection or system override attempt. Access denied.";
    }
    ...
    try {
        if (messages.length > 20) {
            ...
            const summaryPrompt = `Please summarize the following conversation snippet concisely for persistent context. Extract key entities, facts, and the core progression of the dialogue: \n\n${JSON.stringify(toPrune)}`;
            try {
                const summaryResponse = await providerManager.createMessage([{ role: 'user', content: summaryPrompt }], "You are a context assistant.", []);

                // Feature: Index-Linked Memory Tree (Save to Chapter DB)
                MemoryTree.saveChapterSummary(senderId, summaryResponse.text);

                console.log(`[Agent] Successfully archived chapter to Memory Tree.`);
            } catch (e) {
                console.error('[Agent] Context pruning failed, proceeding without summary.');
            }
        }

        return await runAgentLoop(messages, senderId, platform, attachments);
    } finally {
        // Feature: Clean Room Context Sweeper
        // Because raw tool outputs (like HTML scrapes) are never pushed to the persistent DB (only active ephemeral memory),
        // the context naturally self-destructs. We simply need to formally lift the execution taint.
        toolRegistry.clearTaint(contextId);
    }
}
...
async function handleCommand(cmd: string, args: string[], senderId: string, platform: string): Promise<string | null> {
    switch (cmd) {
        ...
        case '/reflect':
            ...
            return await ReflectionEngine.reflect(senderId, platform, topic);
