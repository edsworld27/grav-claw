...
export class OpenRouterProvider implements LLMProvider {
    ...
    async createMessage(messages: MessageParam[], systemPrompt: string, tools: any[]): Promise<LLMResponse> {
        // Ensure system prompt caching for Anthropic models
        const systemMessage = {
            role: 'system',
            content: systemPrompt,
            ...(this.model.includes('anthropic') || this.model.includes('claude') ? { cache_control: { type: 'ephemeral' } } : {})
        };
        ...
        // Cache the last user message as well for maximum savings
        if ((this.model.includes('anthropic') || this.model.includes('claude')) && openAiMessages.length > 2) {
            ...
            if (lastUserMsgIndex !== -1) {
                openAiMessages[lastUserMsgIndex].cache_control = { type: 'ephemeral' };
            }
        }
