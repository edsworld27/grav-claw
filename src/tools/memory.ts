...
export function searchHistoryExecutor(query: string): string {
    ...
    // Sanitize query for FTS5 to prevent malformed match expressions and syntax crashes
    const sanitizedText = query.replace(/[^a-zA-Z0-9\s]/g, ' ').trim();
    if (!sanitizedText) return "Invalid search query: please use simple alphanumeric keywords.";

    // Construct a safe boolean phrasing string for FTS5 (e.g., "word1"* AND "word2"*)
    const ftsQuery = sanitizedText.split(/\\s+/).map(word => `"${word}"*`).join(' AND ');

    try {
        const rows = stmt.all(ftsQuery) as { content: string, timestamp: string }[];
