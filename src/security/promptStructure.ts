...
/**
 * Escape XML special characters and actively destroy fake XML injection tags in untrusted content
 */
function escapeXML(content: string): string {
    // Phase 1: Aggressively destroy structural tags (e.g. </user_message>, <system_instructions>)
    // Even if they try to bypass with spaces: < / user_message >
    let sanitized = content.replace(/<\s*\/?\s*(?:system|instructions|admin|root|ignore|override|prompt|user_message|agent_context|untrusted_user_data|tool_results|security_notice)[^>]*>/gi, '[SANITIZED_TAG]');

    // Phase 2: Standard XML escaping
    return sanitized
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
